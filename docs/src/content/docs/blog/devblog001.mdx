...
Making a data exporter tells me that I have sufficient programming chops, or learning chops, to pull off a simple menu-based game. It's proof that I can make a basic game pipeline from end-to-end.
...
2. **The game objects, such as the concept of a love interest, a stat, a choice**. I used object-oriented programming for these. At this stage I wrote some classes (`LoveInterest`, `Dialog`), then their instances were imported into the game UI. The instances were still in a sense hardcoded, in that they were still difficult to create or update in bulk.
3. **The exporter in Excel**. I wrote an exporter then moved the instances or objects, such as `HIKER_BOYFIE` and his default affection stat, into the exporter. At this stage it was trivial to update values. 
...
Note that you can define more stats, not just affection; any stat you want to keep track of, such as hit points, energy, and so on can be defined.
