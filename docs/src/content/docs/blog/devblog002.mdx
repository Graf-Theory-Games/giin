...
One method of figuring out the number is to build from an atom of gameplay. You'd build the core loop, or even a set of gameplay loops, giving yourself a deadline measured in weeks to less than a quarter to assemble a prototype with placeholders. In this scenario, you haven't figured out how big the game is going to be, nor how long it will take to get to the end, but you have to start somewhere.

Another method is to work backwards, or to begin with the end in mind, which in this case means a complete public beta. I've done this in professional (salaried) environments where a period of experimentation is baked into the costing and is part of a "real" timetable with milestones (deadlines) mapped to distributor and publisher events. The team is usually able to figure out the costing for the game and the size of content based on results during the experimentation phase. This method is not within this blog post's scope.
