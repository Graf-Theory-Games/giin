...
One method to figure out values for stats is to prototype a core loop of gameplay, **working forward**. You'd start prototyping the this loop, and maybe 2-3 other loops, then you'd feed the numbers you've computed on paper to that prototype. I'd suggest a short deadline, not more than a month; this will force you into the habit of trimming any fat before figuring out the meat of your game. In this scenario, you haven't figured out how big the game is going to be, nor how long it will take to get to the end, but you have to start somewhere.
...
You may want to start conservatively (3 to 5 minute game demo) until you build confidence in your experience. Remember that the tutorials of games can be 3 minutes or less and that a starting Pokemon battle against a Lv. 2 Rattata takes about 4 moves—less than a minute.
...
| Character lineup | Atk | Previous HP → New HP |
| -------          | ------ | ------ |
|  Abel            | 5    | 100 → **37** |
|  Benjamin        | 9    | 75 → **12**  |
|  Cain            | 5    | 120 → **57** |
...
Just from reading those numbers and their variance between characters you can already see that there's more tension (and that Benjamin should probably be rebalanced)—and we haven't even touched on other actions, such as Magic, or events such as Miss and Critical.
...
| Unit type      | Rivers 🏞️ | Plains 🌾  | Paved roads 🛣️ |
| --------       | ------    | ---------- | ----------     |
...
| Cavalry 🐎      | -3 tiles  | +3 tiles   | +5 tiles       |
...
Now that we have defined scale levels ("Best", "Terrible", "Good" and so on), we've limited the scope to a subset of numbers. tk define normalization 

| Unit type       | Rivers 🏞️ | Plains 🌾  | Paved roads 🛣️ |
| --------        | ------    | ---------- | ----------     |
| Infantry 🤺     | Bad  | Neutral         | Great       |
| Cavalry 🐎      | Terrible  | Great   | Best       |
| Supply wagon 🐂 | Worst  | Bad   | Great       |
